...
const EMPTY_SUMMARY = { total: 0, average_confidence: 0, sentiment: {}, intent: {}, urgency: {}, products: {}, by_day: {}, low_confidence: 0 }
...
function ReviewTable({ analyses, onFeedback }) {
  ...
  return <section className="panel table-panel">
    ...
    <div className="table-scroll"><table><thead><tr><th>Feedback</th><th>Product</th><th>Sentiment</th><th>Intent</th><th>Confidence</th><th /></tr></thead>
      <tbody>{visible.map((item) => <tr key={item.id}><td><p>{item.text}</p><small>{new Date(item.created_at).toLocaleString()}</small></td><td>{item.product}</td><td><span className={`tag ${item.predictions.sentiment.label}`}>{pretty(item.predictions.sentiment.label)}</span></td><td>{pretty(item.predictions.intent.label)}</td><td>{Math.round(confidence(item) * 100)}%</td><td><button className="text-button" onClick={() => onFeedback(item)}>Correct</button></td></tr>)}</tbody></table></div>
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="top"><header><div><span className="eyebrow">Voice of customer</span><h1>Signals worth acting on.</h1><p>See what customers feel, need, and may do next.</p></div><button className="secondary" onClick={exportCsv} disabled={!analyses.length}>Export CSV</button></header>
      ...
      <section className="grid-two"><Distribution title="Customer sentiment" values={summary.sentiment} colors={['#ff6b57', '#725cff', '#38b98b']} /><Distribution title="Customer intent" values={summary.intent} colors={['#725cff', '#4d8df7', '#f3a83b', '#38b98b']} /></section>
